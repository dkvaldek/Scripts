...
foreach ($dc in $dcList) {
    ...
    try {
        ...
        # >>> Contagem de erros críticos (Error + Critical) <<<
        $crit30 = ($events  | Where-Object { $_.LevelDisplayName -in @('Error','Critical') }).Count
        $crit7  = ($events7 | Where-Object { $_.LevelDisplayName -in @('Error','Critical') }).Count
        ...
        $summary += [PSCustomObject]@{
            ...
            Critical7Days    = $crit7
            Critical30Days   = $crit30
            File7Days        = $out7
            File30Days       = $out30
        }

        Write-Host ("  ✓ 7 dias: {0} eventos ({1} erros críticos) | 30 dias: {2} eventos ({3} erros críticos)" -f $count7, $crit7, $count30, $crit30) -ForegroundColor Cyan
        Write-Host "    Arquivos: "
...
if ($summary.Count -gt 0) {
    $summary |
        Select-Object DomainController,Events7Days,Events30Days,Critical7Days,Critical30Days |
        Sort-Object DomainController |
        Format-Table -AutoSize

    # Totais gerais (somando todos os DCs)
    $totEv7    = ($summary | Measure-Object Events7Days    -Sum).Sum
    $totEv30   = ($summary | Measure-Object Events30Days   -Sum).Sum
    $totCrit7  = ($summary | Measure-Object Critical7Days  -Sum).Sum
    $totCrit30 = ($summary | Measure-Object Critical30Days -Sum).Sum

    Write-Host ""
    Write-Host "=== TOTAIS GERAIS (TODOS OS DCS) ===" -ForegroundColor Yellow
    Write-Host ("  7 dias  : {0} eventos, {1} erros críticos" -f $totEv7,  $totCrit7)
    Write-Host ("  30 dias : {0} eventos, {1} erros críticos" -f $totEv30, $totCrit30)
} else {
